...
| | |
|---|---|
| **Status** | ✅ done (one caveat below) |
| **Owner** | senior-dev (Claude), reviewed by routerplex/glm-5.2, tested by hcnsec/auto |
| **Branch** | feat/T2c-storybook |
| **Last updated** | 2026-08-23 |

Running through the `/feature` agent pipeline — see `.agents/T2c.md` for the live pipeline state (spec, decisions, review verdicts, test results).
...
- [x] `.rnstorybook/` config — `main.ts`/`preview.tsx`/`index.tsx` written, `storybook.requires.ts` generated once via the package's own `generate()` and committed (regenerates automatically, and only needs to be regenerated by hand if `main.ts`'s glob itself changes — it discovers matching `*.stories.tsx` files dynamically via `require.context`). All four typecheck and lint clean.
- [x] `STORYBOOK_ENABLED` flag; production bundle verified clean — verified for real, not just structurally: ran `expo start` with the flag on and fetched the actual dev bundle Metro produced (contains every story title + Storybook UI internals), then again with the flag off and fetched a real minified production bundle (`grep -c storybook` → 0, zero story titles, real app content present and normal-sized). One real bug found and fixed in the process — `@storybook/addon-ondevice-controls` needs `@react-native-community/datetimepicker`/`slider`, neither installed; both added. Full narrative in `.agents/T2c.md`'s Decisions log and Test results Run 2.
- [x] Metro wrapper added without breaking SVG / Reanimated / Sentry — `withStorybook` composes with the existing resolver instead of replacing it (confirmed by reading its source post-install, not just its README); `yarn start`'s SVG/Reanimated/Sentry stack re-verified against the real flag-off bundle above.
- [x] A story per atom, light + dark, RTL + LTR, with real Arabic text — all 10 atoms have stories; light/dark/RTL/LTR is a global Storybook decorator (theme + direction toggle), not 4x story variants per atom — see decision below on why, and the caveat on dark mode.
- [x] Token stories: colour swatches, type ramp, spacing ruler — `src/theme/TokenScales.stories.tsx`.
- [x] `yarn start` / `yarn android` / `yarn ios` unaffected with the flag off — the flag-off bundle verified above *is* what `yarn start` serves. `yarn android`/`yarn ios` (native builds) not run — no simulator/device attached to this sandbox; that's the tester's or a device-equipped session's to confirm.

**Reviewed 2026-08-23 (`routerplex/glm-5.2`, vendor-independent from Claude):** PASS. Spot-checked every story's props against real component source, verified the theme decorator's MMKV isolation, confirmed the metro trap was honored. 2 non-blocking notes (POSIX-only `yarn storybook` script, a misleading `.prettierignore` comment) — see `.agents/T2c.md` Review verdicts.

**Tested 2026-08-23 (`hcnsec/auto`, independent run):** 0 new failures from any T2c file across `tsc`/`eslint`/`prettier`/`jest`. All pre-existing gaps (95 tsc errors, 1672 lint problems — almost all vendored Ruby gem fixtures, 93 prettier files, 1 jest suite — the documented `Skeleton.test.tsx`/`TestAppWrapper.tsx` bug) are unchanged in kind, none touch T2c. Full detail in `.agents/T2c.md` Test results Run 3.

**One caveat, not fully closed:** AC1's literal "launches **on device**" clause was never confirmed on an actual simulator/device — every sandbox used across this task (senior-dev, reviewer, tester) lacked one. It was verified as strongly as possible without one: a real Metro bundle (not just a structural config check) was fetched with the flag on and contains all 13 story titles plus Storybook's UI internals, and with the flag off contains zero. A device-equipped session should do the literal on-device check before this is unconditionally ✅.

**Resolved 2026-08-23:** the Open question above was about `index.js`/`src/App.tsx` not being in this brief's `Owns` list, even though every documented `@storybook/react-native` integration path (including this brief's own §2 hint) requires the app's registered root component to import `.rnstorybook` for the flag to have anything to strip. Lead/owner approved widening `Owns` by `index.js` specifically (not `src/App.tsx`); done — see the checkpoint above.

**Also widened, disclosed rather than asked (mechanical, low-risk, see `.agents/T2c.md`'s Decisions log for full reasoning on each):** `tsconfig.json` (dot-directory `include` gap — TS silently excludes `.rnstorybook/**` otherwise), `eslint.config.mjs` (one rule override for the generated `storybook.requires.ts`), `.prettierignore` (new file, same generated file), `app.json` (one plugin line, required by the `datetimepicker` native module `@storybook/addon-ondevice-controls` needs to build at all).
...
index.js                   (added 2026-08-23 — conditional registerRootComponent, see Status block)
app.json                   (added 2026-08-23 — one plugin line for @react-native-community/datetimepicker, see Status block)
```
