...
| | |
|---|---|
| **Status** | 🔵 in progress (scope question resolved, finishing AC1) |
| **Owner** | senior-dev (Claude) |
...
- [ ] `STORYBOOK_ENABLED` flag; production bundle verified clean — flag + metro-level stripping wired and verified structurally (`metro.config.js` loads cleanly and produces the expected empty/stub `resolveRequest` with the flag on *and* off — checked directly, not just read). **Not** verified against an actual exported bundle (AC3's literal "grep the release bundle") — see the open question below; nothing currently imports `.rnstorybook`, so an `expo export` right now would trivially show no Storybook strings regardless of the flag and wouldn't prove anything.
- [x] Metro wrapper added without breaking SVG / Reanimated / Sentry — `withStorybook` composes with the existing resolver instead of replacing it (confirmed by reading its source post-install, not just its README); `yarn start`'s SVG/Reanimated/Sentry stack re-verified structurally with the flag off.
...
- [x] `yarn start` / `yarn android` / `yarn ios` unaffected with the flag off — verified by loading `metro.config.js` directly with the flag unset: same SVG/Reanimated/Sentry config shape as before, `withStorybook` returns before any of its side effects (telemetry, `storybook.requires` regeneration, channel server) run. Did not run a full device build (no simulator/device attached to this session).

**Resolved 2026-08-23:** the Open question above was about `index.js`/`src/App.tsx` not being in this brief's `Owns` list, even though every documented `@storybook/react-native` integration path (including this brief's own §2 hint) requires the app's registered root component to import `.rnstorybook` for the flag to have anything to strip. Lead/owner approved widening `Owns` by `index.js` specifically (not `src/App.tsx`); senior-dev is finishing the conditional `registerRootComponent` change there now.
