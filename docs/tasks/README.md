...
| T3a | [FocusPlayer](T3a-focus-player.md) | ✅ done (caveats below) | `organisms/FocusPlayer/**` | T2b | T3b |
...
- **2026-08-23** — [T3a](T3a-focus-player.md) executed via the `/feature` pipeline (`.agents/T3a.md`) — `builder` on `routerplex/deepseek-v4-pro` implemented, `reviewer` on `routerplex/MiniMax-M3` ran 4 passes (2 CHANGES_REQUESTED, both fixed and re-verified: AC6/AC7 data-shape and keep-awake-race bugs, then a supplementary pass on a jest-config fix). `VideoModal` deleted; `FocusPlayer` plays `hosted` sources via `expo-video` (chosen over `react-native-video` — first-party under Expo CNG) and `youtube` sources through `react-native-youtube-iframe`, patched via `patch-package` to actually load from `youtube-nocookie.com` (the library's hardcoded `PlayerScripts.js` never exposed YouTube's own `host` IFrame Player API option — `patch-package` was removed in T0 for being unused, reintroduced here scoped to this one library). Found and fixed a real regression along the way: `expo-keep-awake` broke `yarn test` via a missing `transformIgnorePatterns` entry, same class of gap T0 solved for other packages. Two honest caveats, not silently glossed: AC6 (`startSec`/`endSec` clipping) is deferred — `Lesson` has no `source` field until the Family feature lands `LessonSource` on the type — and AC4's literal on-device adversarial test was never run on a real simulator/device, none being available in any sandbox this task touched.
