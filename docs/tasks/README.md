...
- [ ] **Flip `expo.android.edgeToEdgeEnabled`** — deferred, not avoided; targetSdk 36 forces it on Android 15+. See [T1b](T1b-native.md) for the handover rule. Confirmed still live during T0's `expo prebuild` run (2026-08-17): `EDGE_TO_EDGE_PLUGIN: edgeToEdgeEnabled customization is no longer available — Android 16 makes edge-to-edge mandatory.`
- [ ] **Commit the T4 migration.** It's sitting uncommitted on `feat/expo` pending the owner's review.
- [ ] **T0 (2026-08-17), found but not fixed — outside `Owns`:**
  - **`src/screens/auth/ForgotPassword/index.tsx:24`** has the same hardcoded `__DEV__ ? 'm@m.com' : ''` pattern T0 fixed in `Login`. Same file (and `src/screens/auth/signup/index.tsx`) also has the same missing-`.catch` bug T0 fixed as C8 in `Login` — neither `GET`/`POST` call has error handling. Nobody's `Owns` list covers `auth/ForgotPassword` or `auth/signup`.
  - **C3 (MMKV `encryptionKey`) could not be done.** It requires editing `src/store/index.ts` (the `new MMKV()` call), but that file is T2d's per this table (`Owns: src/services/**, src/store/**, src/App.tsx`) — T0's `Owns` list only carves out `src/store/subscriptionSlice.ts`. Recommend T2d picks this up, or the owner explicitly widens T0's scope for one file.
  - **`package-old.json`** is a tracked, stale leftover (duplicate of `package.json` pre-migration). Not in Part D's enumerated deletion list, not referenced anywhere live.
  - **`Fastlane/APPCONST`** has `IOS_APP_SPECIFIER`/`AND_APP_SPECIFIER = 'com.ebda3.binaa'`, which disagrees with `app.json`'s `com.binaa`. Combined with the already-known `Fastlane/Appfile` iOS mismatch (`com.dl.sdccards`), this means C7's Fastfile fix (this pass) can't be verified end-to-end — three different bundle IDs across `app.json`, `Appfile`, and `APPCONST`.
  - **`Gemfile`'s `plugins_path`** points at lowercase `fastlane/Pluginfile`; the real directory is `Fastlane/Pluginfile` (capital F). Works today only because macOS's filesystem is case-insensitive — would break on a case-sensitive Linux CI runner if a Fastlane lane were ever added to CI (T0's CI doesn't run Fastlane, so this didn't block Part E, but it's a live landmine).
  - **`__mocks__/TestAppWrapper.tsx`** imports `queryClient, storage` from `@/App`, neither of which `src/App.tsx` exports (there's no `QueryClientProvider` in `src/App.tsx` yet — see §3). This is why `src/components/atoms/Skeleton/Skeleton.test.tsx` still fails after T0's Jest-harness fixes: `__tests__/App.test.tsx` (T0-owned) now renders the real `src/App.tsx` successfully, but `Skeleton.test.tsx` (T2b's) crashes on this pre-existing wrapper bug. Fixing it needs either a `queryClient`/`storage` export from `src/App.tsx` (barred by T0's own scope note — "Sentry DSN only, do not restructure") or a rewrite of `TestAppWrapper.tsx` itself (not in T0's `Owns`).
